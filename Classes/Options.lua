...
function handleSharedMedia()
	...
	if SP.db then
		SP.Vars.ui.font = sharedMedia:Fetch("font", SP.db.media.options.FONT.value)
		SP.Vars.ui.textures.BAR_TEX = sharedMedia:Fetch("statusbar", SP.db.media.options.BAR.value)
		SP.Vars.ui.textures.PRED_BAR_TEX = sharedMedia:Fetch("statusbar", SP.db.media.options.PRED_BAR.value)
	end
...
function showLayoutGUI()
	...
	local elements = {};
	for k, v in pairs(SP.db.layout.options) do
	...
	reset:SetCallback("OnClick", function() SP.db.layout = lastLayout; ReloadUI(); end)
	reset.frame:Show()
	...
	defaults:SetCallback("OnClick", function() SP.db.layout = SP.getDefaultConfig().layout; ReloadUI(); end)
	defaults.frame:Show()
	...
	updateLayoutSetting = function(value)
		...
		local layoutValues = SP.db.layout.options[value].value;
